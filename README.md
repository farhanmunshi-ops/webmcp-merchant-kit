...
## Verified against the native API (not a polyfill)

Run in Google Chrome 151 with the WebMCP origin-trial features enabled ([scripts/verify-native.mjs](scripts/verify-native.mjs)) — real `document.modelContext`, on the live desk page:

- **23 tools coexist**: Shopify's 10 built-ins (`search_catalog`, `update_cart`, `proceed_to_checkout`, …) + our 13 (6 global, 6 desk, 1 compiled from `webmcp.json`), Shopify's adapter loaded alongside, zero collisions
- Our tools **and** Shopify's execute through the same native `executeTool` (e.g. `get_cart` → "Cart is empty. Next steps: use search_catalog…")
- Two findings folded back into the kit: native `executeTool` takes the **`RegisteredTool` object** from `getTools()`, not a name (the bundled polyfill now mirrors this), and the browser **does not enforce `inputSchema`** — so the kit validates required fields itself before `execute` runs

## Try it

**No agent browser? 60 seconds:** open [packrift.com/pages/agent-desk?demo=1](https://packrift.com/pages/agent-desk?demo=1) in any browser — the page installs a small polyfill, and **"▶ Watch a demo agent work this desk"** runs the real tool sequence (real catalog match, real freight call) in your tab, with every call mirrored in the Agent Desk panel.
...
- [scripts/](scripts/) — native-Chrome and demo-mode verification scripts
- [test/](test/) — kit + manifest compiler tests (`npm test`)
